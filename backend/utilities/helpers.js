...
function time_validate(time) {
  ...
  if (
    !regex.test(time) ||
    !moment(time.substring(0, 10), "YYYY-MM-DD", true).isValid()
  ) {
    return 1;
  } else {
    return 0;
...
async function send_email_by_group(course_id, group_id, subject, body) {
  ...
  for (let row of pg_res_user.rows) {
    let pg_res_email = await db.query(
      "SELECT email FROM user_info WHERE username = ($1)",
      [row["username"]]
    );
...
async function get_criteria_id(course_id, task, criteria) {
  let pg_res = await db.query(
    "SELECT * FROM course_" +
      course_id +
      ".criteria WHERE task = ($1) AND criteria = ($2)",
    [task, criteria]
  );
...
async function get_total_out_of(course_id) {
  let pg_res = await db.query(
    "SELECT task, SUM(total) AS sum FROM course_" +
      course_id +
      ".criteria GROUP BY task",
    []
  );
...
async function format_marks_one_task_csv(json, course_id, task, res, total) {
  ...
  let file_name =
    "marks_" + current_time.format("YYYY-MM-DD-HH-mm-ss") + ".csv";
...
async function gitlab_create_group_and_project_no_user(
  course_id,
  group_id,
  task
) {
  ...
  let pg_res_starter_code_url = await db.query(
    "SELECT starter_code_url FROM course_" +
      course_id +
      ".task WHERE task = ($1)",
    [task]
  );
...
async function gitlab_create_group_and_project_with_user(
  course_id,
  group_id,
  username,
  task
...
async function gitlab_add_user_with_gitlab_group_id(
  gitlab_group_id,
  gitlab_url,
  username
) {
...
async function gitlab_add_user_without_gitlab_group_id(
  course_id,
  group_id,
  username
) {
...
async function gitlab_remove_user(course_id, group_id, username) {
  // Get gitlab_group_id
  let pg_res = await db.query(
    "SELECT gitlab_group_id FROM course_" +
      course_id +
      ".group WHERE group_id = ($1)",
    [group_id]
  );
...
async function gitlab_get_commits(course_id, group_id) {
  ...
  let pg_res = await db.query(
    "SELECT gitlab_project_id FROM course_" +
      course_id +
      ".group WHERE group_id = ($1)",
    [group_id]
  );
...
async function get_max_user_tokens(course_id, username) {
  ...
  let pg_res_user_tokens = await db.query(
    "SELECT token_count FROM course_" +
      course_id +
      ".user WHERE username = ($1)",
    [username]
  );
  if (
    pg_res_default_tokens.rowCount !== 1 ||
    pg_res_user_tokens.rowCount !== 1
  ) {
...
async function get_user_token_usage(course_id, username) {
  ...
  let pg_res_submission = await db.query(
    "SELECT * FROM course_" +
      course_id +
      ".submission WHERE group_id = ANY($1::int[])",
    [groups]
  );
...
async function get_due_date(course_id, group_id) {
  ...
  let pg_res_group = await db.query(
    "SELECT task, extension FROM course_" +
      course_id +
      ".group WHERE group_id = ($1)",
    [group_id]
  );
  ...
  if (task_group_id !== null) {
    let pg_res_task_group = await db.query(
      "SELECT max_token FROM course_" +
        course_id +
        ".task_group WHERE task_group_id = ($1)",
      [task_group_id]
    );
  ...
  if (task_group_id !== null) {
    let pg_res_task_group_all_tasks = await db.query(
      "SELECT task FROM course_" +
        course_id +
        ".task WHERE task_group_id = ($1)",
      [task_group_id]
    );
  ...
  if (pg_res_members.rowCount < 1) {
    return {
      ...
      due_date_with_extension: due_date_with_extension.format(
        "YYYY-MM-DD HH:mm:ss"
      ),
      due_date_with_extension_and_token: due_date_with_extension.format(
        "YYYY-MM-DD HH:mm:ss"
      ),
      token_length: -1,
    };
  ...
  return {
    ...
    due_date_with_extension_and_token: due_date_with_extension_and_token.format(
      "YYYY-MM-DD HH:mm:ss"
    ),
    max_token,
    token_length,
...
async function get_submission_before_due_date(course_id, group_id) {
  ...
  let due_date_with_extension_and_token =
    due_date_data["due_date_with_extension_and_token"];
  ...
  if (last_commit_time_utc !== null) {
    let minutes_past_due_date_with_extension = moment
      .duration(
        last_commit_time_utc.diff(
          moment.tz(due_date_with_extension, "America/Toronto")
        )
      )
      .asMinutes();
    if (minutes_past_due_date_with_extension > 0) {
      token_used = Math.ceil(
        minutes_past_due_date_with_extension / token_length
      );
    }
  }
...
async function collect_all_submissions(course_id, task, overwrite) {
  ...
  for (let row of pg_res.rows) {
    collect_processes.push(
      collect_one_submission(course_id, row["group_id"], overwrite)
    );
  }
...
async function download_all_submissions(course_id, task) {
  ...
  for (let row of pg_res.rows) {
    ...
    let pg_res_commit_id = await db.query(
      "SELECT commit_id FROM course_" +
        course_id +
        ".submission WHERE group_id = ($1)",
      [group_id]
    );
