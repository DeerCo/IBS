...
router.get("/", (req, res) => {
    if (!("task" in req.query) || helpers.string_validate(req.query["task"])) {
        ...
        let sql_mark = "SELECT * FROM course_" + res.locals["course_id"] + ".mark task = ($1) ORDER BY criteria_id";
        client.query(sql_mark, [req.query["task"]], (err, pgRes) => {
            if (err) {
                res.status(404).json({ message: "Unknown error." });
            } else {
                helpers.calculate_marks_one_task(pgRes.rows, res.locals["course_id"], req.query["task"]).then(marks => {
                    res.json({ marks: marks });
                });
            }
        });
