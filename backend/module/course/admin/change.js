...
router.put('/', (req, res) => {
    ...
    if (
        !('gitlab_group_id' in req.body) ||
        (req.body['gitlab_group_id'] !== '' && helpers.number_validate(req.body['gitlab_group_id']))
    ) {
        res.status(400).json({ message: 'The Gitlab group id is missing or not correct.' });
        return;
    }
    if (
        !('default_token_count' in req.body) ||
        helpers.number_validate(req.body['default_token_count'])
    ) {
        res.status(400).json({
            message: 'The default token count is missing or has invalid format.'
        });
        return;
    }
    ...
    let sql_change_data = [
        ...
        req.body['gitlab_group_id'],
        req.body['default_token_count'],
        req.body['token_length'],
        req.body['hidden'],
        req.body['course_id']
    ];
