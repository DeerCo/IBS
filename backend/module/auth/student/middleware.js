...
async function retrieve_data(req) {
    if (req.method === "GET") {
        if ("task" in req.query && !helpers.name_validate(req.query["task"])) {
            ...
            return { task: "", change_group: true, hide_interview: true, interview_group: null };
        }
    } else {
    ...
    return { task: data["task"], change_group: data["change_group"], hide_interview: data["hide_interview"], interview_group: data["interview_group"] };
}
