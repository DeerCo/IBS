...
router.post("/add", (req, res) => {
    ...
    if (!("task_order" in req.body) || helpers.number_validate(req.body["task_order"])) {
        res.status(400).json({ message: "The task order is missing or invalid." });
        return;
    }

    let due_date = req.body["due_date"] + " America/Toronto";
    let sql_add = "INSERT INTO course_" + res.locals["course_id"] + ".task (task, due_date, hidden, min_member, max_member, task_order) VALUES (($1), ($2), ($3), ($4), ($5), ($6))";
