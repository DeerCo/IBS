...
router.post("/invite", (req, res) => {
	...
	let sql_add_user = "INSERT INTO course_" + res.locals["course_id"] + ".course_group_user (course_task_id, username, group_id, status) VALUES (($1), ($2), ($3), 'pending')";
	...
	client.query(sql_select_user, [req.body["group_id"], res.locals["user_name"]], (err, pgRes) => {
		...
		client.query(sql_select_group, [req.body["group_id"]], (err, pgRes) => {
			if (err) {
				...
				client.query(sql_add_user, [course_task_id, req.body["username"], req.body["group_id"]], (err, pgRes) => {
					if (err) {
